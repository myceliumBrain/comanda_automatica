...
// ── ZOOM ──
;(function () {
  const ZOOM_KEY = 'bar_julio_zoom';
  const MIN = 0.6, MAX = 1.6, STEP = 0.1;
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    wrap.innerHTML =
      '<button class="zoom-btn" id="zbtn-in"  title="Zoom in">＋</button>' +
      '<button class="zoom-btn" id="zbtn-out" title="Zoom out">－</button>';
    document.body.appendChild(wrap);

    document.getElementById('zbtn-in').addEventListener('click', function () {
      applyZoom(Math.min(MAX, Math.round((getZoom() + STEP) * 10) / 10));
    });
    document.getElementById('zbtn-out').addEventListener('click', function () {
...
// ── NÚMERO DE COMANDA ──
// Derivado do histórico: varre todos os dias e retorna o maior num + 1.

async function _maxNumHistorico() {
  const datas = await window.api.listarDatasHistorico();
  if (!datas.length) return 0;
  const porDia = await Promise.all(datas.map(d => window.api.lerHistorico(d)));
  let max = 0;
  porDia.flat().forEach(p => {
...
async function proximoNumComanda() {
  return (await _maxNumHistorico()) + 1;
}
...
async function atualizarHintUltimaComanda() {
  ...
  if (!hint) return;
  const max = await _maxNumHistorico();
  hint.textContent = max > 0 ? `última: #${max}` : '';
}
...
function criarAgendamento({ cliente, data, obs, horaPedido }) {
