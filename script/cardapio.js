...
function adicionarItem(tipo) {
  ...
  const preco  = parseFloat((document.getElementById(precoId).value || '0').replace(',', '.')) || 0;
  const lista  = tipo === 'prato' ? cardapio.pratos : cardapio.bebidas;
...
function abrirModal(tipo, id) {
  ...
  document.getElementById('edit-preco').value = (item.preco ?? 0).toFixed(2).replace('.', ',');
...
function salvarEdicao() {
  ...
  item.nome            = nome;
  item.preco           = parseFloat((document.getElementById('edit-preco').value || '0').replace(',', '.')) || 0;
